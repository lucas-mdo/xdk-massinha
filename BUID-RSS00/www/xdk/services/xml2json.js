...
function X2JS() {
  ...
  function getNodeLocalName( node ) {
    ...
    //if(!nodeLocalName) // Yeah, this is IE!!
    //  nodeLocalName = node.baseName;
    if(!nodeLocalName || nodeLocalName === '') // =='' is IE too
      ...
      if(node.nodeType == DOMNodeTypes.ELEMENT_NODE) {
        ...
        for(var cidx = 0; cidx <nodeChildren.length; cidx++) {
          ...
          if(!result[childName]) {
            ...
            var aridx = 0;
            while(result[childName][aridx]) aridx++;
  ...
  function endTag(jsonObj,elementName) {
    return '</'+ (jsonObj.__prefix? (jsonObj.__prefix+':'):'')+elementName+'>';
  }
